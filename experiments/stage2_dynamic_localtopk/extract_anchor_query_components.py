...
from open_clip.grade_stage2_v2 import forward_visual_multilevel, parse_selected_layers, visual_num_layers, DiseaseAnchorQueryStage2Refiner
...
def main():
 ...
 ref=DiseaseAnchorQueryStage2Refiner(anchors,num_heads=cargs.get('num_heads',8),alpha=cargs.get('alpha',0.5)).to(device); ref.load_state_dict(sd,strict=True)
 ...
 with torch.no_grad():
  for im,p in tqdm(dl,desc='extract-anchor-query-components',ncols=100):
 ...
 (outd/'components.json').write_text(json.dumps({'input':args.input,'refiner_checkpoint':args.refiner_checkpoint,'num_samples':len(paths),'alpha_checkpoint':float(ref.alpha.detach().cpu()),'missing':len(msg.missing_keys),'unexpected':len(msg.unexpected_keys),'refiner_type':'disease_anchor_query'},indent=2),encoding='utf-8')

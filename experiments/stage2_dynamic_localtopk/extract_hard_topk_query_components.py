...
from open_clip.grade_stage2_v2 import forward_visual_multilevel, parse_selected_layers, visual_num_layers, HardTopKQueryStage2Refiner
...
def main():
 ...
 ref=HardTopKQueryStage2Refiner(anchors,num_heads=cargs.get('num_heads',8),topk=cargs.get('topk',4),alpha=cargs.get('alpha',0.5)).to(device); ref.load_state_dict(sd,strict=True)
 ...
 with torch.no_grad():
  for im,p in tqdm(dl,desc='extract-hard-topk-query-components',ncols=100):
 ...
 (outd/'components.json').write_text(json.dumps({'input':args.input,'refiner_checkpoint':args.refiner_checkpoint,'num_samples':len(paths),'alpha_checkpoint':float(ref.alpha.detach().cpu()),'missing':len(msg.missing_keys),'unexpected':len(msg.unexpected_keys),'refiner_type':'hard_topk_query'},indent=2),encoding='utf-8')

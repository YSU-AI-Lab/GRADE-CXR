...
from open_clip.grade_stage2_v2 import forward_visual_multilevel, parse_selected_layers, visual_num_layers, TopKMaskedDiseaseAnchorQueryStage2Refiner
...
def main():
 ap=argparse.ArgumentParser(); ap.add_argument('--input',required=True); ap.add_argument('--output-dir',required=True); ap.add_argument('--stage1-checkpoint',required=True); ap.add_argument('--refiner-checkpoint',required=True); ap.add_argument('--model',default=DEFAULT_MODEL); ap.add_argument('--pretrained',default=DEFAULT_PRETRAINED); ap.add_argument('--selected-layers',default='3,6,9,12'); ap.add_argument('--batch-size',type=int,default=64); ap.add_argument('--workers',type=int,default=2); ap.add_argument('--device',default='cuda'); ap.add_argument('--img-key',default='Image Index'); ap.add_argument('--alpha-override',type=float,default=None)
 ...
 ck=torch.load(args.refiner_checkpoint,map_location='cpu',weights_only=False); sd=ck.get('state_dict',ck); anchors=sd['anchors']; cargs=ck.get('args',{})
 ref=TopKMaskedDiseaseAnchorQueryStage2Refiner(anchors,num_heads=cargs.get('num_heads',8),topk=cargs.get('topk',4),alpha=cargs.get('alpha',0.5)).to(device); ref.load_state_dict(sd,strict=True)
 if args.alpha_override is not None: ref.alpha.data.fill_(float(args.alpha_override))
 ref.eval()
 ...
 with torch.no_grad():
  for im,p in tqdm(dl,desc='extract-topk-masked-anchor-components',ncols=100):
 ...
 (outd/'components.json').write_text(json.dumps({'input':args.input,'refiner_checkpoint':args.refiner_checkpoint,'num_samples':len(paths),'alpha_checkpoint':float(ref.alpha.detach().cpu()),'missing':len(msg.missing_keys),'unexpected':len(msg.unexpected_keys),'refiner_type':'topk_masked_disease_anchor_query'},indent=2),encoding='utf-8')

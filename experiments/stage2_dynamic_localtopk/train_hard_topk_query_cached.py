...
from open_clip.grade_stage2_v2 import HardTopKQueryStage2Refiner, build_localtopk_oracle_teacher, predictive_localtopk_stage2_losses
class DS(Dataset):
 def __init__(self,c):
...
def save(p,model,args,ep,row,load):
 p.parent.mkdir(parents=True,exist_ok=True); torch.save({'state_dict':model.state_dict(),'epoch':ep,'metrics':row,'args':vars(args),'stage1_load_info':load,'refiner_type':'hard_topk_query'},p)
def main():
 ...
 a,load=anchors(args,device); ref=HardTopKQueryStage2Refiner(a.cpu(),num_heads=args.num_heads,topk=args.topk,alpha=args.alpha).to(device); opt=torch.optim.AdamW(ref.parameters(),lr=args.lr,weight_decay=args.weight_decay); rows=[]
 (out/'config_resolved.json').write_text(json.dumps({**vars(args),'cache_meta':ds.meta},indent=2),encoding='utf-8')
 for ep in range(1,args.epochs+1):
  ref.train(); sums=[]
  for batch in tqdm(dl,desc=f'hard-topk-query ep{ep}',ncols=100):

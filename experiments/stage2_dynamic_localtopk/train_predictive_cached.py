...
from open_clip.grade_stage2_v2 import PredictiveLocalTopKStage2Refiner, build_localtopk_oracle_teacher, predictive_localtopk_stage2_losses
class DS(Dataset):
 def __init__(self,c): c=Path(c); self.z=np.load(c/'z_base.float16.npy',mmap_mode='r'); self.t=np.load(c/'multi_tokens.float16.npy',mmap_mode='r'); self.y=np.load(c/'labels14.float32.npy',mmap_mode='r'); self.meta=json.loads((c/'cache_meta.json').read_text())
...
def save(p,model,args,ep,row,load): p.parent.mkdir(parents=True,exist_ok=True); torch.save({'state_dict':model.state_dict(),'epoch':ep,'metrics':row,'args':vars(args),'stage1_load_info':load,'refiner_type':'predictive_localtopk'},p)
def main():
 ap=argparse.ArgumentParser(); ap.add_argument('--cache-dir',required=True); ap.add_argument('--stage1-checkpoint',required=True); ap.add_argument('--output-dir',required=True); ap.add_argument('--model',default=DEFAULT_MODEL); ap.add_argument('--pretrained',default=DEFAULT_PRETRAINED); ap.add_argument('--epochs',type=int,default=3); ap.add_argument('--batch-size',type=int,default=512); ap.add_argument('--workers',type=int,default=2); ap.add_argument('--lr',type=float,default=5e-4); ap.add_argument('--weight-decay',type=float,default=0.01); ap.add_argument('--seed',type=int,default=42); ap.add_argument('--device',default='cuda'); ap.add_argument('--topk',type=int,default=4); ap.add_argument('--alpha',type=float,default=0.5); ap.add_argument('--lambda-z',type=float,default=2.0); ap.add_argument('--lambda-bce',type=float,default=1.0); ap.add_argument('--lambda-pres',type=float,default=0.2)
 ...
 a,load=anchors(args,device); ref=PredictiveLocalTopKStage2Refiner(a.cpu(),topk=args.topk,alpha=args.alpha).to(device); opt=torch.optim.AdamW(ref.parameters(),lr=args.lr,weight_decay=args.weight_decay); rows=[]
 (out/'config_resolved.json').write_text(json.dumps({**vars(args),'cache_meta':ds.meta},indent=2),encoding='utf-8')
 for ep in range(1,args.epochs+1):
  ref.train(); sums=[]
  for batch in tqdm(dl,desc=f'predictive-localtopk ep{ep}',ncols=100):
